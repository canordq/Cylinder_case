...
from Env2D import Environment2D
...
environment_base = Environment2D(simu_name = simu_name, do_baseline=False, continue_training=True, ENV_ID=-1, host="environment1", node=nodelist[1]) # Baseline
...
environments = [Environment2D(simu_name = simu_name, do_baseline=False, continue_training=True, ENV_ID=i, host="environment{}".format(i+1), node=nodelist[i+1]) for i in range(num_servers)]
